        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            <el-row>
              <el-card>
                <div>
                  共计 {{sumData.pageCount}} 个页面, 累计访客 {{sumData.user}} 位
                </div>
              </el-card>
            </el-row>
                ...
                >
                  ...
                  >
                    <thead>
                      <th
                        :key="h"
                        v-for="h in trendTable.thead"
                      >{{h}}</th>
                    </thead>
                    <tbody>
                      <tr
                        :key="i"
                        v-for="(row,i) in trendTable.tbody"
                      >
                        <td
                          :key="k"
                          v-for="(val,k) in row"
  ...
  methods: {
    async getPUVTrendByES () {
      ...
      var result = await Server({
        url: 'statistics/getProjectReport',
        method: 'post',
        data: query
      }).then(({ data }) => {
        return data.data || {}
      ...
      this.sumData['pageCount'] = result.count
      this.trend = trend
    }
  }
}
